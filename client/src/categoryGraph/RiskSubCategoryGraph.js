...
const RiskSubCategoryGraph = () => {
    ...
    const fetchSubCategories = async () => {
      try {
        const riskResponse = await axios.get(
          API.RISKSUBCATEGORYREPORT_RISK_API(category)
        );
        setUser(riskResponse.data);

        const response = await axios.get(
          API.RISKSUBCATEGORY_RISK_API(category)
        );
  ...
  const chartLabels = subCategories.map(
    (subCategory) => subCategory.subcategory
  );
  const chartData = subCategories.map(
    (subCategory) => subCategory.subcategory_count
  );
    ...
    <div className="chart-container">
      ...
      {subCategories.length > 0 ? (
        <div className="centered-chart">
          ...
          />
          {hoveredBar !== null && <div className="hover-message">Click Me</div>}
        </div>
      ) : (
        <p></p>
      )}
