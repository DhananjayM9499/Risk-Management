...
const RiskGroupGraph = () => {
    ...
    const fetchRiskGroup = async () => {
      try {
        const riskResponse = await axios.get(
          API.RISKGROUPREPORT_RISK_API(subcategory)
        );
        setUser(riskResponse.data);

        const response = await axios.get(
          API.RISKGROUPGRAPH_RISK_API(subcategory)
        );
  ...
  const handleRiskGroupClick = (_, elements) => {
    if (elements && elements.length > 0) {
      ...
      const newTab = window.open(
        `/riskseveritygraph/${encodeURIComponent(clickedRiskGroup)}`,
        "_blank"
      );
      newTab.focus();
    }
  ...
  return (
    <div className="chart-container">
      <Header />
      <h2>{`Riskgroup for ${subcategory}`}</h2>
      {riskGroup.length > 0 ? (
