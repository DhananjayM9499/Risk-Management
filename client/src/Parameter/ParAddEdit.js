...
const ParAddEdit = () => {
  ...
  useEffect(() => {
    if (parameterid) {
      axios
        .get(API.VIEW_PARAMETER_API(parameterid))
  ...
  const handlSubmit = (e) => {
    e.preventDefault();
    if (!categorycode || !parametercode || !evidence || !fileupload) {
      toast.error("please provider value into each input field");
    } else {
      if (!parameterid) {
        axios
          .post(API.ADD_PARAMETER_API, {
            categorycode,
            parametercode,
            evidence,
            fileupload,
        ...
        axios
          .put(API.UPDATE_PARAMETER_API(parameterid), {
            categorycode,
            parametercode,
            evidence,
            fileupload,
